...
public class gc_winchester_m1895_3006_customModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack, ItemStack parent, @Nullable LivingEntity entity, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        ...
        poseStack.mulPose(Vector3f.ZN.rotationDegrees(-30F * boltPivot));
        poseStack.mulPose(Vector3f.XN.rotationDegrees(-15F * boltPivot));
        poseStack.translate(-(boltMovement * 1.8) * 0.0625, 0, 0);
        poseStack.translate(0, 5.8 * 0.0625, 0);
        RenderUtil.renderModel(SpecialModels.GC_WINCHESTER_M1895_3006_MAIN.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);

        poseStack.pushPose();
        poseStack.translate(0, -5.8 * 0.0625, 0);
        poseStack.translate(0, 0, (boltMovement * 3.5) * 0.0625);
        poseStack.translate(0, 5.8 * 0.0625, 0);
        RenderUtil.renderModel(SpecialModels.GC_WINCHESTER_M1895_BOLT.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
        poseStack.popPose();

        poseStack.pushPose();
        poseStack.translate(0, -5.8 * 0.0625, 0);
        poseStack.mulPose(Vector3f.XN.rotationDegrees(-15F * Math.min(boltPivot*2F,1)));
        poseStack.translate(0, 5.8 * 0.0625, 0);
        RenderUtil.renderModel(SpecialModels.GC_WINCHESTER_M1895_BOLT1.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
        poseStack.popPose();

        poseStack.pushPose();
        poseStack.translate(0, -6.9 * 0.0625, 3.1 * 0.0625);
        poseStack.mulPose(Vector3f.XN.rotationDegrees(-90F * Math.min(boltPivot*2F,1)));
        poseStack.translate(0, 6.9 * 0.0625, -3.1 * 0.0625);
        RenderUtil.renderModel(SpecialModels.GC_WINCHESTER_M1895_LEVER.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
        poseStack.popPose();
