...
public class gc_m3_carbine_customModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack, ItemStack parent, @Nullable LivingEntity entity, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        ...
        {
            RenderUtil.renderModel(SpecialModels.GC_M2_CARBINE_PARTS_MAIN.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
        }

        ItemStack attachmentStack1 = Gun.getAttachment(IAttachment.Type.UNDER_BARREL, stack);
        if(!attachmentStack1.isEmpty())
        {
            RenderUtil.renderModel(SpecialModels.GC_M3_CARBINE_TACTICAL.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
        }

        ItemStack attachmentStack2 = Gun.getAttachment(IAttachment.Type.SCOPE, stack);
        if(!attachmentStack2.isEmpty())
        {
            RenderUtil.renderModel(SpecialModels.GC_M21_SIGHT_RAIL.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
        }
        poseStack.pushPose();
        poseStack.translate(0, -5.8 * 0.0625, 0);
        poseStack.translate(0, 0, cooldown/8);
        poseStack.translate(0, 5.8 * 0.0625, 0);
        RenderUtil.renderModel(SpecialModels.GC_M1_CARBINE_BOLT.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
        poseStack.popPose();
    }
