...
public class gc_ots33_tactical_customModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack, ItemStack parent, @Nullable LivingEntity entity, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        ...
        {
            RenderUtil.renderModel(SpecialModels.GC_OTS33_TAC.getModel(), stack, poseStack, buffer, light, overlay);
        }

        ItemStack attachmentStack2 = Gun.getAttachment(IAttachment.Type.BARREL, stack);
        if(!attachmentStack2.isEmpty())
        {
            RenderUtil.renderModel(SpecialModels.GC_OTS33_SILENCER.getModel(), stack, poseStack, buffer, light, overlay);
        }
        ...
        if (Gun.hasAmmo(stack)){
            ...
            poseStack.translate(0, 5.8 * 0.0625, 0);
            RenderUtil.renderModel(SpecialModels.GC_OTS33_BOLT.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
            poseStack.popPose();
        } else {
