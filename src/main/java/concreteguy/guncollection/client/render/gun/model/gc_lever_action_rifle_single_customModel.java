...
public class gc_lever_action_rifle_single_customModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack, ItemStack parent, @Nullable LivingEntity entity, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        ...
        poseStack.translate(0, 5.8 * 0.0625, 0);
        RenderUtil.renderModel(SpecialModels.GC_LEVER_ACTION_RIFLE_SINGLE_MAIN.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);

        poseStack.pushPose();
        poseStack.translate(0, -6.9 * 0.0625, 3.1 * 0.0625);
        poseStack.mulPose(Vector3f.XN.rotationDegrees(-40F * Math.min(boltPivot*2F,1)));
        poseStack.translate(0, 6.9 * 0.0625, -3.1 * 0.0625);
        RenderUtil.renderModel(SpecialModels.GC_LEVER_ACTION_RIFLE_SINGLE_BOLT.getModel(), transformType, null, stack, parent, poseStack, buffer, light, overlay);
        poseStack.popPose();
